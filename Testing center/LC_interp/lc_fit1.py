...
for i in range(len(t_bin)-1):
    t_bin_m.append(np.sqrt(t_bin[i]*t_bin[i+1]))
    t_bin_n.append(t_bin[i]-t_bin_m[i])
    t_bin_p.append(t_bin[i+1]-t_bin_m[i])
no_of_specs=7
base_name="spec"
...
cf=(norm*662.5)/(file_count_r*corr_fact)
ph_interp = lambda tt: interp1d(np.log10(t_bin_m),ph_idx,fill_value="extrapolate")(np.log10(tt))
ph_new=ph_interp(time)
flux=cf*np.power(10,-(ph_new-1))*cnt_data
# ff=[]
# for i in range(no_of_specs):
