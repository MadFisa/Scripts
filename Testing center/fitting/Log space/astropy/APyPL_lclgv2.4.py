...
lc_data=lc_file[:,3]
lc_err=lc_file[:,4] 
log_lc_err=np.log10(1+(lc_err/lc_data))
##############################################################################
...
popt, pcov = curve_fit(model_f, np.log10(t_data), np.log10(lc_data),p0=pp,bounds=(LB,UB))

# popt, pcov = curve_fit(model_f,np.log10(t_data), np.log10(lc_data),sigma=log_lc_err,p0=(-5,2,15,2,3,4),bounds=(LB,UB))
popt2, pcov2 = curve_fit(model_f,np.log10(t_data), np.log10(lc_data),sigma=log_lc_err,p0=(-5,2,15,2,3,4),bounds=(LB,UB))
