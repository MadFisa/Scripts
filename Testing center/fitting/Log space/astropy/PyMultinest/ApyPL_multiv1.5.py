...
def prior(cube):
    ...
    # cube[1] = cube[1]*-10  + 5     #m_f
    cube[1] = cube[1]*15 + 25   # nu_b
    cube[2] = cube[2]*5         # m_b 
    cube[3] =  cube[3]*10 - 5 # alpha_1
    cube[4] =  cube[4]*10 - 5# alpha_2
    # if ndim < 7:
    #     return
    #    	cube[7] =cube[7]*4# s=uniform prior between 0 and 4
    return cube
...
def loglike(cube):
    ...
    likelihood = np.sum(-0.5*(((lc_model- lc_data)/lc_err)**2))- (0.5*np.sum(np.log(2.*np.pi*lc_err**2)))
    # print(cube)
    print(likelihood)
    # likelihood = ((-0.5*(lc_model- log_lc_data))**2).sum()
    # likelihood = exp(-0.5 * ((pos - positions) / width)**2) / (2*pi*width**2)**0.5
...
result=pymultinest.solve(LogLikelihood=loglike,Prior=prior,n_dims= n_params, outputfiles_basename='test/'+GRB + '_fit_',
	resume=False,verbose = True)
