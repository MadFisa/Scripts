...
def f_nu(t,nu,F_0,m_f,nu_b0,m_b,alpha_1t,alpha_2t,delta_t):
...
def model_ph(t,F_0,m_f,nu_b0,m_b,alpha_1,alpha_2):
    ...
    ph_idx_t=calc_phIdx_evln(t,nu,F_0,m_f,nu_b0,m_b,alpha_1t,alpha_2t,delta_t)
...
popt, pcov = curve_fit(model_ph, t_data, ph_data,bounds=(LB,UB))


t_plot=np.logspace(4,7)

n=len(t_plot)
ph_fit=model_ph(t_plot,*popt)
...
plt.scatter(t_data,ph_data,label="data")
plt.plot(t_plot,ph_fit,label="fit")
plt.legend()
plt.xscale("log")
# plt.yscale("log")
